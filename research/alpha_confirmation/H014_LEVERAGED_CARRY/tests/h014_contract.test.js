...
describe('H014 Confirmatory Contract & Cryptographic Lock Tests', () => {
  const baseDir = path.resolve(process.cwd(), 'research/alpha_confirmation/H014_LEVERAGED_CARRY');
  ...
  it('G-LOCK-02: Preregistration Lock exists and is valid', () => {
    ...
    expect(lock.hypothesisId).toBe('H014');
  ...
  it('G-LOCK-04: Confirmatory Charter specifies non-adaptive single-hypothesis protocol (M=1)', () => {
    const charterPath = path.join(baseDir, 'charter/H014_CONFIRMATORY_CHARTER.md');
    const content = fs.readFileSync(charterPath, 'utf8');
    expect(content).toContain('M = 1');
    expect(content).toContain('AnnReturn_net >= +10.0%');
    expect(content).toContain('Sharpe >= 5.00');
    expect(content).toContain('MaxDD <= 3.00%');
