/**
 * ALPHA FACTORY — AD011 TRADFI MULTI-TIMEFRAME ALPHA DISCOVERY RUNNER
 * Script: run_ad011_discovery.js
 * 
 * Objectives:
 * 1. Evaluates 24 MTF Liquidity Sweep cells across TradFi (SPY, QQQ, EURUSD, GBPUSD, GLD) & Crypto (BTCUSDT).
...
import { AD011MTFEngine } from '../core/ad011_mtf_engine.js';
...
async function main() {
  console.log('================================================================');
  console.log('🏛️ ALPHA FACTORY — PROGRAM AD011: TRADFI MTF ALPHA DISCOVERY');
  console.log('================================================================\n');
  ...
  const specPath = path.resolve(rootDir, 'research/alpha_discovery/AD011/spec/AD011_CAMPAIGN_SPEC.json');
  ...
  console.log('--- Executing MTF Simulation & 14-Day Calendar Block Bootstrap ---');
  for (let i = 0; i < spec.cells.length; i++) {
    ...
    const simRes = AD011MTFEngine.simulate(assetCandles, cell, friction);
  ...
  const outResultsPath = path.resolve(rootDir, 'research/alpha_discovery/AD011/discovery/AD011_DISCOVERY_RESULTS.json');
  ...
  const manifestPath = path.resolve(rootDir, 'research/alpha_discovery/AD011/discovery/AD011_DISCOVERY_MANIFEST.json');
...
function generateDiscoveryReport(results, eligible) {
  const reportPath = path.resolve(rootDir, 'research/alpha_discovery/AD011/discovery/AD011_DISCOVERY_REPORT.md');

  let md = `# 🏛️ AD011 DISCOVERY REPORT — TRADFI MULTI-TIMEFRAME ALPHA\n\n`;
  md += `**Programa:** AD011  \n`;
  ...
  md += `O programa **AD011** investigou formalmente a existência de alfa causal proveniente de **Varreduras de Liquidez em 4H (Macro Sweeps)** confirmadas por **Deslocamento CISD em 1H** em um universo diversificado de 6 ativos institucionais: Índices de Ações EUA (\`SPY\`, \`QQQ\`), Forex G10 (\`EURUSD\`, \`GBPUSD\`), Macro Commodity (\`GLD\`) e Benchmark de Controle Cripto (\`BTCUSDT\`).\n\n`;
  md += `Foram simuladas **24 células combinatórias** no período estrito de Discovery (2023-11-21 a 2024-12-31), aplicando fricções realistas de mercado, **Block Bootstrap de 14 dias (B=10.000)** e penalidade de multiplicidade estrita de **Benjamini-Yekutieli (BY, 2001)**.\n\n`;
  ...
  md += `Para que uma hipótese seja elegível para a Fase de Confirmação (Holdout 2025–2026), ela precisa atender rigorosamente a todos os 5 gates:\n`;
  ...
  if (eligible.length > 0) {
    ...
    md += `Nenhuma das 24 variações atingiu cumulativamente os 5 gates. As causas estruturais observadas nos dados são detalhadas no arquivo forense.\n`;
  }
